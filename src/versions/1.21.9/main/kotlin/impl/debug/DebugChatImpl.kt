...
internal actual class DebugChatScreen actual constructor(val messages: List<Pair<Instant, Component>>) : Screen(CommonComponents.EMPTY) {
    ...
    override fun mouseClicked(event: MouseButtonEvent, meow: Boolean): Boolean {
        if (event.buttonInfo().isLeft) {
            this.layout.visitWidgets {
                if (event.x.toInt() in it.x until it.x + it.width && event.y.toInt() in it.y until it.y + it.height) {
                    it.onClick(event, meow)
                }
            }
        }
        return super.mouseClicked(event, meow)
    }
...
private class Widget(timestamp: Instant, val content: Component) : StringWidget(
    ...
    override fun onClick(e: MouseButtonEvent, m: Boolean) {
        val copyType: String
